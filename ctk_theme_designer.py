...
class Controller():
    ...
    def __init__(self):
        ...
        self.app.title('CTk Theme Designer')
        ...
        initial_display = [f for f in os.listdir(self._json_dir) if f.endswith('.json')]
    ...
    def _launch_preferences(self):
        ...
        top_prefs.title('CTk Theme Designer Preferences')
        # Make preferences dialog modal
        top_prefs.grab_set()

        btn_json_dir = ctk.CTkButton(master=top_prefs, text='Set JSON location', command=self._json_dir_selection)
        btn_json_dir.grid(row=0, column=0, padx=5, pady=5)

        btn_close = ctk.CTkButton(master=top_prefs, text='Close', command=top_prefs.destroy)
        btn_close.grid(row=2, column=0, padx=5, pady=(15, 5))
        self._save_winfo()

    def _json_dir_selection(self):
        self._json_dir = Path(tk.filedialog.askdirectory(initialdir=self._json_dir))
    ...
    def _load_theme(self, dummy=None):
        ...
        self._theme_file = self.opm_theme.get()
...
class Preview():
    ...
    def __init__(self, theme='green', appearance_mode='Dark'):
        ...
        self._render_disabled = tk.IntVar(value=1)
